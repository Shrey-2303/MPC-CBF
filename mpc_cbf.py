...
class MPC:
    ...
    def set_tvp_for_mpc(self, mpc):
        ...
        def tvp_fun_mpc(t_now):
            ...
            if self.moving_obstacles_on is True:
                for i in range(len(self.moving_obs)):
                    tvp_struct_mpc['_tvp', :, 'x_moving_obs'+str(i)] = self.moving_obs[i][0] * t_now**2 + self.moving_obs[i][1] * t_now + self.moving_obs[i][2]
                    tvp_struct_mpc['_tvp', :, 'y_moving_obs'+str(i)] = self.moving_obs[i][3] * t_now**2 + self.moving_obs[i][4] * t_now + self.moving_obs[i][5]
